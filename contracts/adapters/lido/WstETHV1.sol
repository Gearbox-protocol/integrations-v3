...
contract WstETHV1Adapter is AbstractAdapter, IwstETHV1Adapter {
    ...
    constructor(address _creditManager, address _wstETH)
        AbstractAdapter(_creditManager, _wstETH) // U:[WST-1]
    {
        stETH = IwstETH(_wstETH).stETH(); // U:[WST-1]
        wstETHTokenMask = _getMaskOrRevert(_wstETH); // U:[WST-1]
        stETHTokenMask = _getMaskOrRevert(stETH); // U:[WST-1]
    }
    ...
    function wrap(uint256 amount)
        external
        override
        creditFacadeOnly // U:[WST-2]
        returns (uint256 tokensToEnable, uint256 tokensToDisable)
    {
        (tokensToEnable, tokensToDisable) = _wrap(amount, false); // U:[WST-3]
    }
    ...
    function wrapAll()
        ...
        returns (uint256 tokensToEnable, uint256 tokensToDisable)
    {
        address creditAccount = _creditAccount(); // U:[WST-4]

        uint256 balance = IERC20(stETH).balanceOf(creditAccount); // U:[WST-4]
        if (balance > 1) {
            unchecked {
                (tokensToEnable, tokensToDisable) = _wrap(balance - 1, true); // U:[WST-4]
            }
        }
    }
    ...
    function _wrap(uint256 amount, bool disableStETH)
        ...
        returns (uint256 tokensToEnable, uint256 tokensToDisable)
    {
        _approveToken(stETH, type(uint256).max); // U:[WST-3,4]
        _execute(abi.encodeCall(IwstETH.wrap, (amount))); // U:[WST-3,4]
        _approveToken(stETH, 1); // U:[WST-3,4]
    ...
    function unwrap(uint256 amount)
        ...
        returns (uint256 tokensToEnable, uint256 tokensToDisable)
    {
        (tokensToEnable, tokensToDisable) = _unwrap(amount, false); // U:[WST-5]
    }
    ...
    function unwrapAll()
        ...
        returns (uint256 tokensToEnable, uint256 tokensToDisable)
    {
        address creditAccount = _creditAccount(); // U:[WST-6]

        uint256 balance = IERC20(targetContract).balanceOf(creditAccount); // U:[WST-6]
        if (balance > 1) {
            unchecked {
                (tokensToEnable, tokensToDisable) = _unwrap(balance - 1, true); // U:[WST-6]
            }
        }
    }
    ...
    function _unwrap(uint256 amount, bool disableWstETH)
        ...
        returns (uint256 tokensToEnable, uint256 tokensToDisable)
    {
        _execute(abi.encodeCall(IwstETH.unwrap, (amount))); // U:[WST-5,6]
