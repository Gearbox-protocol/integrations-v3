...
contract BalancerV2VaultAdapter is AbstractAdapter, IBalancerV2VaultAdapter {
    AdapterType public constant _gearboxAdapterType = AdapterType.BALANCER_VAULT;
        ...
        creditFacadeOnly
    {
        ...
        _execute(abi.encodeCall(IBalancerV2Vault.joinPool, (poolId, creditAccount, creditAccount, request)));
                ...
                abi.encodeCall(
                    ...
                    (
                        ...
                        _getJoinSingleAssetRequest(
                            poolId, assetIn, balanceInBefore, (balanceInBefore * minRateRAY) / RAY
                            )
                    )
                ),
                true
        ...
        creditFacadeOnly
    {
        ...
        _execute(abi.encodeCall(IBalancerV2Vault.exitPool, (poolId, creditAccount, payable(creditAccount), request)));

        _enableAssets(creditAccount, request.assets, _getBalancesFilter(creditAccount, request.assets));
    }
                ...
                abi.encodeCall(
                    ...
                    (
                        ...
                        _getExitSingleAssetRequest(
                            poolId, assetOut, balanceInBefore, (balanceInBefore * minRateRAY) / RAY
                            )
                    )
                ),
                true
    ...
    function _enableAssets(address creditAccount, IAsset[] memory assets, int256[] memory filter) internal {
        uint256 len = assets.length;

        for (uint256 i = 0; i < len;) {
            if (filter[i] < -1) {
                creditManager.checkAndEnableToken(creditAccount, address(assets[i]));
            }
