...
import {
    ...
    PendleStatus,
    PendleTokenType
} from "./interfaces/IPendleRouterAdapter.sol";
import {
    IPendleRouter,
    IPendleMarket,
    IYToken,
    ...
    ApproxParams,
    LimitOrderData
} from "./interfaces/external/IPendleRouter.sol";
...
contract PendleRouterAdapter is AbstractAdapter, IPendleRouterAdapter {
    ...
    function addLiquiditySingleToken(
        ...
        if (isPairAllowed[market][input.tokenIn][market] != PendleStatus.ALLOWED) {
    ...
    function addLiquiditySingleTokenDiff(
        ...
        if (isPairAllowed[market][diffInput.tokenIn][market] != PendleStatus.ALLOWED) {
            revert PairNotAllowedException();
        }
