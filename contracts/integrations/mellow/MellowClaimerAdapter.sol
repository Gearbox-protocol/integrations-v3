...
import {IMellowClaimer} from "./interfaces/external/IMellowClaimer.sol";
import {
    IMellowMultiVault,
    IEigenLayerWithdrawalQueue,
    Subvault,
    MellowProtocol
} from "./interfaces/external/IMellowMultiVault.sol";
import {IMellowClaimerAdapter, MellowMultiVaultStatus} from "./interfaces/IMellowClaimerAdapter.sol";
import {MellowWithdrawalPhantomToken} from "./MellowWithdrawalPhantomToken.sol";
...
contract MellowClaimerAdapter is AbstractAdapter, IMellowClaimerAdapter {
    ...
    function withdrawPhantomToken(address stakedPhantomToken, uint256 amount) external creditFacadeOnly returns (bool) {
        ...
        returns (uint256[] memory subvaultIndices, uint256[][] memory withdrawalIndices)
    {
        ...
        for (uint256 i = 0; i < subvaultCount; i++) {
            ...
            if (subvault.protocol == MellowProtocol.EIGEN_LAYER) {
                (, withdrawalIndices[i],) = IEigenLayerWithdrawalQueue(subvault.withdrawalQueue)
                    .getAccountData({
                    account: user,
