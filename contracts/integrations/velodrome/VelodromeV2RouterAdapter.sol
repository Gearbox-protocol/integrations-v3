...
import {IVelodromeV2Router, Route} from "./interfaces/external/IVelodromeV2Router.sol";
import {
    IVelodromeV2RouterAdapter,
    VelodromeV2PoolStatus,
    VelodromeV2Pool
} from "./interfaces/IVelodromeV2RouterAdapter.sol";
        ...
        for (uint256 i; i < len; ++i) {
            ...
            if (pools[i].allowed) {
                ...
                _hashToPool[poolHash] = VelodromeV2Pool({
                    token0: token0, token1: token1, stable: pools[i].stable, factory: pools[i].factory
                });
            } else {
