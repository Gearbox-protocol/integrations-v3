...
import {IBalancerV3Router} from "./interfaces/external/IBalancerV3Router.sol";
import {IBalancerV3Pool} from "./interfaces/external/IBalancerV3Pool.sol";
...
contract BalancerV3RouterGateway is IBalancerV3Router, IVersion {
    ...
    function swapSingleTokenExactIn(
        ...
        IBalancerV3Router(balancerV3Router)
            .swapSingleTokenExactIn(pool, tokenIn, tokenOut, exactAmountIn, minAmountOut, deadline, wethIsEth, userData);
    ...
    function addLiquidityUnbalanced(
        ...
        for (uint256 i; i < tokens.length; ++i) {
            ...
            IPermit2(permit2)
                .approve(address(tokens[i]), balancerV3Router, uint160(exactAmountsIn[i]), uint48(block.timestamp));
        }

        IBalancerV3Router(balancerV3Router)
            .addLiquidityUnbalanced(pool, exactAmountsIn, minBptAmountOut, wethIsEth, userData);
    ...
    function removeLiquiditySingleTokenExactIn(
        ...
        IBalancerV3Router(balancerV3Router)
            .removeLiquiditySingleTokenExactIn(pool, exactBptAmountIn, tokenOut, minAmountOut, wethIsEth, userData);
