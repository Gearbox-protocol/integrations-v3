...
import {ITraderJoeRouter, Path, Version} from "./interfaces/external/ITraderJoeRouter.sol";
import {ITraderJoeRouterAdapter, TraderJoePoolStatus, TraderJoePool} from "./interfaces/ITraderJoeRouterAdapter.sol";
        ...
        for (uint256 i; i < len; ++i) {
            ...
            if (pools[i].allowed) {
                ...
                _hashToPool[poolHash] = TraderJoePool({
                    token0: token0, token1: token1, binStep: pools[i].binStep, poolVersion: pools[i].poolVersion
                });
            } else {
