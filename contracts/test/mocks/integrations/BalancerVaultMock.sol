...
contract BalancerVaultMock is IBalancerV2Vault {
    ...
    function addPool(
        ...
        address pool = address(
            new BPTMock("Balancer Pool Token", "BPT", 18, weights, poolId)
        );
    ...
    function addStablePool(
        ...
        address pool = address(
            new BPTStableMock("Balancer Stable Pool Token", "BSPT", 18, poolId)
        );
    ...
    function swap(
        ...
        require(
            poolData[singleSwap.poolId].pool != address(0),
            "BalancerVault: Unknown pool"
        );

        uint256 rate = poolData[singleSwap.poolId].ratesRAY[
            address(singleSwap.assetIn)
        ][address(singleSwap.assetOut)];

        require(rate != 0, "BalancerVault: Rate not set");

        amountCalculated = singleSwap.kind == SwapKind.GIVEN_IN
            ? (singleSwap.amount * rate) / RAY
            : (singleSwap.amount * RAY) / rate;

        if (singleSwap.kind == SwapKind.GIVEN_IN) {
            amountCalculated =
                (singleSwap.amount *
                    rate *
                    (10000 - uint256(poolData[singleSwap.poolId].fee))) /
                (RAY * 10000);

            require(
                amountCalculated >= limit,
                "BalancerVault: GIVEN_IN output below limit"
            );

            IERC20(address(singleSwap.assetIn)).transferFrom(
                funds.sender,
                address(this),
                singleSwap.amount
            );
            IERC20(address(singleSwap.assetOut)).transfer(
                funds.recipient,
                amountCalculated
            );
        } else {
            amountCalculated =
                (singleSwap.amount * RAY * 10000) /
                (rate * (10000 - uint256(poolData[singleSwap.poolId].fee)));

            require(
                amountCalculated <= limit,
                "BalancerVault: GIVEN_OUT input above limit"
            );
    ...
    function batchSwap(
        ...
        for (uint256 i = 0; i < assetDeltas.length; ++i) {
            require(
                assetDeltas[i] <= limits[i],
                "BalancerVault: BatchSwap output outside limits"
            );

            if (assetDeltas[i] < 0) {
                IERC20(address(assets[i])).transfer(
                    funds.recipient,
                    uint256(-assetDeltas[i])
                );
            } else if (assetDeltas[i] > 0) {
                IERC20(address(assets[i])).transferFrom(
                    funds.sender,
                    address(this),
                    uint256(assetDeltas[i])
                );
    ...
    function queryBatchSwap(
        ...
        if (kind == SwapKind.GIVEN_IN) {
            for (uint256 i = 0; i < swaps.length; ++i) {
                require(
                    poolData[swaps[i].poolId].pool != address(0),
                    "BalancerVault: Unknown pool"
                );
    ...
    function joinPool(
        ...
        require(
            sender == recipient && sender == msg.sender,
            "BalancerVault: Unsupported sender or recipient"
        );
        ...
        if (kind == JoinKind.EXACT_TOKENS_IN_FOR_BPT_OUT) {
            ...
            ) = _exactTokensInForBPTOut(request.userData);

            uint256 bptOut;

            for (uint256 i = 0; i < request.assets.length; ++i) {
                require(
                    amountsIn[i] <= request.maxAmountsIn[i],
                    "BalancerVault: Asset deposit exceeds limit"
                );

                IERC20(address(request.assets[i])).transferFrom(
                    sender,
                    address(this),
                    amountsIn[i]
                );
                ...
                bptOut +=
                    (amountsIn[i] *
                        poolData[poolId].depositRatesRAY[
                            address(request.assets[i])
                        ]) /
                    RAY;
            }
            ...
            require(
                bptOut >= minBPTAmountOut,
                "BalancerVault: Insufficient BPT out amount"
            );

            BPTMock(poolData[poolId].pool).mint(recipient, bptOut);
        } else if (kind == JoinKind.TOKEN_IN_FOR_EXACT_BPT_OUT) {
            (uint256 bptAmountOut, uint256 tokenIndex) = _tokenInForExactBPTOut(
                request.userData
            );
            ...
            require(
                amountIn <= request.maxAmountsIn[tokenIndex],
                "BalancerVault: Asset deposit exceeds limit"
            );

            IERC20(address(request.assets[tokenIndex])).transferFrom(
                sender,
                address(this),
                amountIn
            );
            ...
            for (uint256 i = 0; i < nAssets; ++i) {
                ...
                require(
                    amountIn <= request.maxAmountsIn[i],
                    "BalancerVault: Asset deposit exceeds limit"
                );
    ...
    function exitPool(
        ...
        if (kind == ExitKind.EXACT_BPT_IN_FOR_ONE_TOKEN_OUT) {
            (uint256 bptAmountIn, uint256 tokenIndex) = _exactBptInForTokenOut(
                request.userData
            );
            ...
            require(
                amountOut >= request.minAmountsOut[tokenIndex],
                "BalancerVault: Insufficient asset amount out"
            );
            ...
            for (uint256 i = 0; i < nAssets; ++i) {
                ...
                require(
                    amountOut >= request.minAmountsOut[i],
                    "BalancerVault: Insufficient asset amount out"
                );
            ...
            ) = _bptInForExactTokensOut(request.userData);
            ...
            for (uint256 i = 0; i < nAssets; ++i) {
                require(
                    amountsOut[i] >= request.minAmountsOut[i],
                    "BalancerVault: Insufficient asset amount out"
                );
            ...
            require(
                bptIn <= maxBPTAmountIn,
                "BalancerVault: BPT amount burned larger than limit"
            );
