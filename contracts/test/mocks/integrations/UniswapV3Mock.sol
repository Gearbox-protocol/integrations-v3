...
contract UniswapV3Mock is ISwapRouter, DSTest {
    ...
    mapping(address => mapping(address => mapping(uint24 => uint256)))
        public rates;
        ...
        returns (uint256 amountOut)
    {
        ...
        while (path.length >= ADDR_SIZE * 2 + FEE_SIZE) {
            (address tokenA, address tokenB, uint24 fee) = path
                .decodeFirstPool();
            ...
            amountOut =
                (((amountOut * rate) / RAY) * ((1_000_000 - uint256(fee)))) /
                (1_000_000);
    ...
    function quoteExactInputSingle(
        ...
        require(rate != 0, "UniswapV3Mock: Rate is not setup");
        amountOut =
            (((amountIn * rate) / RAY) * ((1_000_000 - uint256(fee)))) /
            (1_000_000);
    }
