...
import {ISwapRouter} from "../../../../integrations/uniswap/interfaces/external/IUniswapV3.sol";
import {
    IUniswapV3Adapter,
    IUniswapV3AdapterTypes,
    UniswapV3PoolStatus,
    UniswapV3Pool
} from "../../../../integrations/uniswap/interfaces/IUniswapV3Adapter.sol";
...
contract UniswapV3AdapterUnitTest is AdapterUnitTestHelper, IUniswapV3AdapterTypes {
    ...
    function test_U_UNI3_03_exactInputSingle_works_as_expected() public {
        ...
        _readsActiveAccount();
        _executesSwap({
            tokenIn: tokens[0], callData: abi.encodeCall(ISwapRouter.exactInputSingle, (params)), requiresApproval: true
        });
    ...
    function test_U_UNI3_05_exactInput_works_as_expected() public {
        ISwapRouter.ExactInputParams memory params = ISwapRouter.ExactInputParams({
            path: _makePath(0), amountIn: 123, amountOutMinimum: 456, deadline: 789, recipient: creditAccount
        });
        vm.expectRevert(IUniswapV3Adapter.InvalidPathException.selector);
        vm.prank(creditFacade);
        ...
        _readsActiveAccount();
        _executesSwap({
            tokenIn: tokens[0], callData: abi.encodeCall(ISwapRouter.exactInput, (params)), requiresApproval: true
        });
    ...
    function test_U_UNI3_06_exactDiffInput_works_as_expected() public diffTestCases {
        ...
        ExactDiffInputParams memory params = ExactDiffInputParams({
            path: _makePath(0), deadline: 789, leftoverAmount: diffLeftoverAmount, rateMinRAY: 0.5e27
        });
        vm.expectRevert(IUniswapV3Adapter.InvalidPathException.selector);
        vm.prank(creditFacade);
    ...
    function test_U_UNI3_08_exactOutput_works_as_expected() public {
        ISwapRouter.ExactOutputParams memory params = ISwapRouter.ExactOutputParams({
            path: _makePath(0), amountOut: 123, amountInMaximum: 456, deadline: 789, recipient: creditAccount
        });
        vm.expectRevert(IUniswapV3Adapter.InvalidPathException.selector);
        vm.prank(creditFacade);
