...
contract ConvexV1BoosterAdapterUnitTest is AdapterUnitTestHelper {
    ...
    function test_U_CVX1B_03_deposit_works_as_expected() public {
        for (uint256 i; i < 2; ++i) {
            ...
            _executesSwap({
                tokenIn: tokens[0], callData: abi.encodeCall(adapter.deposit, (42, 1000, stake)), requiresApproval: true
            });
    ...
    function test_U_CVX1B_05_withdraw_works_as_expected() public {
        _executesSwap({
            tokenIn: tokens[1], callData: abi.encodeCall(adapter.withdraw, (42, 1000)), requiresApproval: false
        });
