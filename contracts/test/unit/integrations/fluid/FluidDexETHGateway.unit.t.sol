...
import {FluidDexETHGateway} from "../../../../integrations/fluid/FluidDexETHGateway.sol";
import {
    IFluidDex,
    ConstantViews,
    Implementations
} from "../../../../integrations/fluid/interfaces/external/IFluidDex.sol";
...
contract FluidDexPoolMock is IFluidDex {
    ...
    function swapIn(bool swap0to1, uint256 amountIn, uint256, address to) external payable returns (uint256 amountOut) {
        if (swap0to1) {
            if (token0 == ETH) {
