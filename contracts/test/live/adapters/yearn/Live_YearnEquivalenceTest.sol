...
contract Live_YearnEquivalenceTest is LiveTestHelper {
    ...
    function test_live_YET_01_Yearn_adapters_and_original_contracts_are_equivalent() public {
        for (uint256 i = 0; i < yearnVaults.length; ++i) {
            uint256 snapshot0 = vm.snapshot();

            address vaultAdapter = getAdapter(address(creditManager), yearnVaults[i]);

            if (vaultAdapter == address(0)) {
            ...
            tokenTestSuite.approve(token, creditAccount, supportedContracts.addressOf(yearnVaults[i]));
            ...
            compareBehavior(
                creditAccount,
                getAdapter(address(creditManager), yearnVaults[i]),
                10 ** IERC20Metadata(token).decimals(),
                true,
                comparator
            );
