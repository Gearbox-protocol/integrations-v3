...
contract WstETHV1AdapterTest is DSTest, AdapterTestHelper {
    ...
    function _openStETHTestCreditAccount(
        bool wrap
    ) internal returns (address creditAccount, uint256 amount) {
        ...
        if (wrap) {
            executeOneLineMulticall(
                address(adapter),
                abi.encodeCall(adapter.wrapAll, ())
            );
    ...
    function test_AWSTV1_03_wrap_and_unwrap_if_uses_has_no_account() public {
        ...
        executeOneLineMulticall(
            address(adapter),
            abi.encodeCall(adapter.wrap, (1000))
        );
        ...
        executeOneLineMulticall(
            address(adapter),
            abi.encodeCall(adapter.unwrapAll, ())
        );
        ...
        executeOneLineMulticall(
            address(adapter),
            abi.encodeCall(adapter.unwrap, (1000))
        );
    }
