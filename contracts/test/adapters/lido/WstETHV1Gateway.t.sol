...
contract WstETHV1GatewayTest is DSTest {
    ...
    function test_WSTGV1_01_constructor_sets_parameters_correctly() public {
        assertEq(address(wstETHGateway.wstETH()), tokenTestSuite.addressOf(Tokens.wstETH), "Incorrect wstETH tokenb ");

        assertEq(wstETHGateway.stETH(), tokenTestSuite.addressOf(Tokens.STETH), "Incorrect STETH token");

        assertEq(wstETHGateway.pool(), address(pool), "Incorrect pool tokenb ");

        assertEq(IERC20(wstETHGateway.stETH()).allowance(address(wstETHGateway), address(wstETH)), type(uint256).max);
    }
    ...
    function test_WSTGV1_03_add_and_remove_liquidity_works_correctly() public {
        ...
        assertEq(IERC20(dt).balanceOf(USER), wstETHexpectedAmount, "Incorrect amount deposited");
        assertEq(tokenTestSuite.balanceOf(Tokens.STETH, USER), 0, "INCORRECT BALANCE");
        ...
        uint256 expectedAmount =
            (wstETH.getStETHByWstETH(30 * WAD) * (PERCENTAGE_FACTOR - pool.withdrawFee())) / PERCENTAGE_FACTOR;

        assertEq(tokenTestSuite.balanceOf(Tokens.STETH, USER), expectedAmount, "INCORRECT STETH BALANCE");
    }
}
