...
contract Live_MetapoolTest is DSTest, LiveEnvHelper {
    ...
    function compareBehavior(address curvePoolAddr, address accountToSaveBalances, bool isAdapter) internal {
        if (isAdapter) {
            ...
            evm.prank(USER);
            creditFacade.multicall(multicallBuilder(pool.exchange(1, 0, 3000 * WAD, 2000 * WAD)));
            comparator.takeSnapshot("after_exchange", accountToSaveBalances);

            evm.prank(USER);
            creditFacade.multicall(multicallBuilder(pool.exchange_underlying(0, 2, 500 * WAD, 125 * (10 ** 6))));
            ...
            evm.prank(USER);
            creditFacade.multicall(multicallBuilder(pool.exchange_all(1, 0, RAY / 2)));
            comparator.takeSnapshot("after_exchange_all", accountToSaveBalances);

            evm.prank(USER);
            creditFacade.multicall(multicallBuilder(pool.add_all_liquidity_one_coin(0, RAY / 2)));
            comparator.takeSnapshot("after_add_all_liquidity_one_coin", accountToSaveBalances);

            evm.prank(USER);
            creditFacade.multicall(multicallBuilder(pool.remove_all_liquidity_one_coin(0, RAY / 2)));
            comparator.takeSnapshot("after_remove_all_liquidity_one_coin", accountToSaveBalances);

            evm.prank(USER);
            creditFacade.multicall(multicallBuilder(pool.exchange_all_underlying(0, 1, RAY / 2)));
            ...
            evm.prank(USER);
            pool.exchange(1, 0, 3000 * WAD, 2000 * WAD);
            comparator.takeSnapshot("after_exchange", accountToSaveBalances);

            evm.prank(USER);
            pool.exchange_underlying(0, 2, 500 * WAD, 125 * (10 ** 6));
            comparator.takeSnapshot("after_exchange_underlying", accountToSaveBalances);
