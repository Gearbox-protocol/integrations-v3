...
contract Live_CurveSusdEquivalenceTest is DSTest, LiveEnvHelper {
    ...
    function compareBehavior(
        ...
        ICurvePool4Assets curvePool = ICurvePool4Assets(curvePoolAddr);
        ICurvePool4Assets curveDeposit = ICurvePool4Assets(curveDepositAddr);

        evm.prank(USER);
        curvePool.exchange(0, 1, 2000 * WAD, 1500 * (10**6));
        ...
        evm.prank(USER);
        curveDeposit.remove_liquidity_one_coin(500 * WAD, 1, 0);
        comparator.takeSnapshot(
            "after_remove_liquidity_one_coin",
            accountToSaveBalances
        );
    ...
    function compareExtraFunctions(
        ...
        if (isAdapter) {
            ...
            evm.prank(USER);
            deposit.remove_all_liquidity_one_coin(0, RAY / 2);
            comparator.takeSnapshot(
                "after_remove_all_liquidity_one_coin",
                accountToSaveBalances
            );
    ...
    function test_live_CRVET_07_SUSD_adapter_and_normal_account_works_identically()
        public
        liveOnly
    {
