...
contract Live_YearnEquivalenceTest is DSTest, LiveEnvHelper {
    ...
    function compareBehavior(
        ...
        if (isAdapter) {
            ...
            evm.prank(USER);
            creditFacade.multicall(
                multicallBuilder(vault.deposit(100 * baseUnit))
            );
            comparator.takeSnapshot(
                "after_deposit_uint256_address",
                accountToSaveBalances
            ...
            evm.prank(USER);
            creditFacade.multicall(
                multicallBuilder(vault.withdraw(10 * baseUnit))
            );
            comparator.takeSnapshot(
                "after_withdraw_uint256",
                accountToSaveBalances
    ...
    function test_live_YET_01_Yearn_adapters_and_original_contracts_are_equivalent()
        public
        liveOnly
    {
